            ...
            VStack {
                Form {
                    ...
                    Section("Общая стоимость") {
                        if let limitTotal = event.limitTotal {
                            LabeledContent("Общая стоимость (руб.)", value: limitTotal, format: .number)
                        }
                    }
...
#Preview {
    EventLimitView(event: Event.example)
}
